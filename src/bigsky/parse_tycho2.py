...
ROOT = HERE.parent.resolve().parent.resolve()
...
# The recommended star designation contains a hyphen between the TYC numbers, e.g. TYC 1-13-1.
# https://vizier.cds.unistra.fr/viz-bin/VizieR-5?-ref=VIZ66587c10273fd&-out.add=.&-source=I/259/tyc2&recno=2166992
...
def to_j2000(ra_degrees, dec_degrees, ra_mas_per_year, dec_mas_per_year):
    ...
    star = Star(ra_hours=ra_degrees/15,
            ...
            try:
                ...
                if hip:
                    ...
                    hip_stars[hip].append(
                        [tyc, hip, ccdm, parse_float(mag, 2), parse_float(bv, r=2), ra, dec]
                    )
                else:
                    writer.writerow(
                        [tyc, hip, "", parse_float(mag, 2), parse_float(bv, r=2), ra, dec]
                    )
